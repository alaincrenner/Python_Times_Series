...
y_train = train2['Dernier']
X_train = train2.drop(["Dernier", 'Date'], axis=1)

# Extracting target variable and features for the testing set
y_test = test2['Dernier']
X_test = test2.drop(["Dernier", 'Date'], axis=1)
...
test2['Dernier_Prediction'] = reg.predict(X_test)

future_prediction = reg.predict(future_data)
future_data['Dernier_Prediction'] = future_prediction
future_data['Date'] = date_range

mse = mean_squared_error(test2['Dernier'], test2['Dernier_Prediction'])
mae = mean_absolute_error(test2['Dernier'], test2['Dernier_Prediction'])
mape = mean_absolute_percentage_error(test2['Dernier'], test2['Dernier_Prediction'])
...
train2['Dernier'].plot(style='k', figsize=(10, 5), label='Actual')
test2['Dernier_Prediction'].plot(style='r', figsize=(10, 5), label='prediction')
